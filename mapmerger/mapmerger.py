...
if len(argv) == 2:
	try:
		ENOUGH_FOR_MERGE = int(argv[1])
	except ValueError:
		print('invalid argument "{}"'.format(argv[1]))
print('idential tiles enough for merge = {}'.format(ENOUGH_FOR_MERGE))
...
for i,dir1 in enumerate(dirs_list):
	...
	for dir2 in dirs_list[i+1:]:
		...
		for f1 in filter(ispng,files(dir1path)):
			for f2 in filter(ispng,files(dir2path)):
				...
				if f1hash.digest() == f2hash.digest():
					...
					if (ENOUGH_FOR_MERGE > 0) and (same_hash_cnt >= ENOUGH_FOR_MERGE):
						break
			if diff_shift:
				break
			if (ENOUGH_FOR_MERGE > 0) and (same_hash_cnt >= ENOUGH_FOR_MERGE):
		...
		if not diff_shift:
			...
			if (ENOUGH_FOR_MERGE <= 0) or (same_hash_cnt >= ENOUGH_FOR_MERGE):
				if pdx != 0 or pdy != 0:
